...
function TCalendar(props) {
    ...
    useEffect(() => {
        const fetchCalendarData = async () => {
            try {
                const res = await DetailApi.getNextReserve(code, new Date().getFullYear(), new Date().getMonth() + 1);
                if (res.data.statusCode === 200) {
                    setSelect(res.data.results.check_list.reserve_day_in_month);
                    setIsCasting(res.data.results.check_list.is_info_casting);
                    setFirstDay(res.data.results.calendar_list[0].date);
                    setIsAlwaysAvailable(res.data.results.calendar_list[0].year === 9999);
                    isCasting && setIsTimeCasting(res.data.results.check_list.is_info_time_casting);
                } else {
                    console.log('error');
                    console.log(res);
    ...
    useEffect(() => {
        const fetchCalendarData = async () => {
            try {
                const currentMonthRes = await DetailApi.getNextReserve(code, year, month);
                if (currentMonthRes.data.statusCode === 200) {
                    setSelect(currentMonthRes.data.results.check_list.reserve_day_in_month);
                    setIsCasting(currentMonthRes.data.results.check_list.is_info_casting);
                    setFirstDay(currentMonthRes.data.results.calendar_list[0].date);
                    setIsAlwaysAvailable(currentMonthRes.data.results.calendar_list[0].year === 9999);

                    // 다음 달 예매 가능 여부 확인
                    const nextMonthRes = await DetailApi.getNextReserve(code, year, month + 1);
                    setNextMonthAvailable(nextMonthRes.data.results.check_list.reserve_day_in_month.length > 0);
                } else {
                    console.log('error');
    ...
    return (
        <SideWrap>
            ...
            {isAlwaysAvailable ? (
                <Styleside>
                    <div className='always-available-container'>
                        <p>상시 상품입니다</p>
                        <button className='pay-button' type='button' onClick={openModal}>예매하기</button>
                    </div>
                </Styleside>
                        ...
                        <div className='calendar-container'>
                            <Calendar
                                ...
                                tileDisabled={({date, view}) => {
                                    if (view === 'month') {
                                        return !select.find((x) => moment(x).format('YYYY-MM-DD') === moment(date).format("YYYY-MM-DD"));
                                    }
                                }}
                                nextLabel={nextMonthAvailable ? undefined : null}
                            />
                        </div>
                    )}
                    <hr/>
                                ...
                                {reserveList && reserveList.map(reserve => {
                                    return (
                                        ...
                                        <div key={reserve.index}>
                                            <div>
                                                <button
                                                    className={isDateSameReserveDate(new Date(), date, reserve.hour, reserve.minute) ? 'button-disabled' : 'button select'}
                                                    onClick={onClickTurn} name='turn1' type='button'>
                                                    {reserve.turn}회 {reserve.hour}:{reserve.minute === 0 ? '00' : reserve.minute}
                                                </button>
                                            </div>
                                            {reserve.reserve_seat_time && reserve.reserve_seat_time.map(seat => {
                                    ...
                                    return (
                                        ...
                                        <div key={reserve.index}>
                                            <div>
                                                <button
                                                    className={isDateSameReserveDate(new Date(), date, reserve.hour, reserve.minute) ? 'button-disabled' : 'button select'}
                                                    onClick={onClickTurn} name='turn2' type='button'>
                                                    {reserve.turn}회 {reserve.hour}:{reserve.minute === 0 ? '00' : reserve.minute}
                                                </button>
                                            </div>
                                            {reserve.reserve_seat_time && reserve.reserve_seat_time.map(seat => {
            ...
            {modalOpen && <PayPopup
                ...
                header={<PopupHeader index={index}/>}
                body={<PopupContent userInfo={userInfo}
                                    date={isAlwaysAvailable ? '상시 상품' : selectDay}
                                    cancelday={isAlwaysAvailable ? '예매 당일' : cancelday}
                                    seat={seat}
                                    seatIndex={isAlwaysAvailable ? [] : (turn === 1 && reserveList[0] ? reserveList[0].reserve_seat_time : (reserveList[1] ? reserveList[1].reserve_seat_time : []))}
                                    hour={isAlwaysAvailable ? '' : (turn === 1 && hour[0] ? hour[0] : (hour[1] || ''))}
                                    minute={isAlwaysAvailable ? '' : (turn === 1 ? (minute[0] === 0 ? '00' : minute[0]) : (minute[1] === 0 ? '00' : (minute[1] || '')))}
                                    turn={isAlwaysAvailable ? '상시' : (turn === 1 ? (reserveList[0] ? reserveList[0].turn : '') : (reserveList[1] ? reserveList[1].turn : ''))}
                                    title={title} index={index}
                                    nextClick={handleIsNextClick} backClick={handleisBackClick}
